...
pub fn build_base_system_prompt(config: &AgentConfig, opts: &PromptOptions) -> String {
    ...
    prompt.push_str(concat!(
        ...
        "```\n",
        "[target=#all msg=a1b2c3d4 time=2026-03-15T01:00:00] @richard: hello everyone\n",
        "[target=#all msg=e5f6a7b8 time=2026-03-15T01:00:01 type=agent] @Alice: hi there\n",
        "[target=dm:@richard msg=c9d0e1f2 time=2026-03-15T01:00:02] @richard: hey, can you help?\n",
        "[target=#all:a1b2c3d4 msg=f3a4b5c6 time=2026-03-15T01:00:03] @richard: thread reply\n",
        ...
        "- **Thread targets** have a colon and short ID suffix: `#all:a1b2c3d4` (thread in #all) or `dm:@richard:x9y8z7a0` (thread in a DM).\n",
        "- When you receive a message from a thread (the target has a `:shortid` suffix), **always reply using that same target** to keep the conversation in the thread.\n",
        "- **Start a new thread**: Use the `msg=` field from the header as the thread suffix. ",
        "For example, if you see `[target=#all msg=a1b2c3d4 ...]`, reply with `send_message(target=\"#all:a1b2c3d4\", content=\"...\")`. ",
        "The thread will be auto-created if it doesn't exist yet.\n",
        "- When you send a message, the response includes the message ID. You can use it to start a thread on your own message.\n",
        "- You can read thread history: `read_history(channel=\"#all:a1b2c3d4\")`\n",
        ...
        "### Formatting — No HTML\n\n",
        "Never output raw HTML tags in your messages. Use plain-text @mentions (e.g. `@alice`) and #channel references (e.g. `#all`, `#t1`). Do NOT wrap them in `<a>` tags or any other HTML.\n\n",
