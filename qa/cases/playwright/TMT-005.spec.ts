...
import { agentNames, ensureMixedRuntimeTrio, ensureStubTrio, createTeamApi, teamExists } from './helpers/api'
...
test.describe('TMT-005', () => {
  ...
  test('Team Member Add / Remove @case TMT-005', async ({ page }) => {
    ...
    await test.step('Steps 2–3: Add bot-b if missing', async () => {
      const row = page.locator('.team-settings-member').filter({ hasText: agents.b })
      if (!(await row.isVisible().catch(() => false))) {
        await page.locator('[role="dialog"] [role="combobox"][aria-label="Add Member"]').click()
        await page.locator('[role="option"]').filter({ hasText: agents.b }).first().click()
      ...
      await expect(page.locator('.team-settings-member').filter({ hasText: agents.b })).toBeVisible()
    })
    ...
    await test.step('Step 4: Members rail lists bot-b', async () => {
      ...
      await expect(page.locator('.members-panel-name').filter({ hasText: agents.b })).toBeVisible()
      await page.locator('.members-panel-close').click()
    })

    if (!skipRealLLM) {
      await test.step(`Step 5: Message in #qa-eng (${agents.b} wakeup)`, async () => {
        const token = `tmt5-${Date.now()}`
        await sendChatMessage(page, `${agents.b} acknowledge ${token}`)
      })
    }
    ...
    await test.step('Steps 6–7: Remove bot-b from settings', async () => {
      ...
      await expect(page.locator('.team-settings-member').filter({ hasText: agents.b })).toHaveCount(0)
      await page.locator('[role="dialog"] button:has-text("Close")').click()
    })
