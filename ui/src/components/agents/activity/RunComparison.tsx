...
function CategoryChips({ categories }: { categories: Record<string, number> }) {
  ...
  if (entries.length === 0)
    return <span className="rc-run-tools">0 tools</span>;
  return (
...
): { toolName: string; event: TraceEventRecord }[] {
  return events
...
function matchRuns(
  ...
  const leftByTool = new Map<
    string,
    { ordinal: number; event: TraceEventRecord }[]
  >();
  const rightByTool = new Map<
    string,
    { ordinal: number; event: TraceEventRecord }[]
  >();
    ...
    <div className="rc-selector">
      ...
      {loading && (
        <div className="rc-loading">
          <Loader2 size={14} className="rc-spinner" /> Loading runs…
        </div>
      )}
      ...
      <div className="rc-run-list">
        {runs.map((run) => {
          ...
          const dur =
            ts.duration > 0 ? `${Math.round(ts.duration / 1000)}s` : "—";
          return (
            <label
              key={run.runId}
              className={`rc-run-item${checked ? " selected" : ""}`}
            >
              <input
                type="checkbox"
                checked={checked}
...
function ComparisonView({
  ...
  const matched = useMemo(
    () => matchRuns(leftEvents, rightEvents),
    ...
    <div className="rc-comparison">
      ...
      <div className="rc-columns-header">
        <div className="rc-col-label">
          {new Date(leftRun.createdAt).toLocaleString([], {
            month: "short",
            day: "numeric",
            hour: "2-digit",
            minute: "2-digit",
        ...
        <div className="rc-col-label">
          {new Date(rightRun.createdAt).toLocaleString([], {
            month: "short",
            day: "numeric",
            hour: "2-digit",
            minute: "2-digit",
      ...
      <div className="rc-rows">
        {matched.map((row, i) => {
          ...
          const rightDur = row.right
            ? getDuration(row.right, rightEvents)
            : null;
          const diffMs =
            leftDur != null && rightDur != null ? rightDur - leftDur : null;
          ...
          return (
            <div key={i} className="rc-row">
              <div className={`rc-cell${row.left ? "" : " rc-cell-empty"}`}>
                {row.left && (
                  <>
                    ...
                    {leftDur != null && (
                      <span className="rc-cell-dur">
                        {formatDuration(leftDur)}
                      </span>
                    )}
                  </>
                )}
              ...
              <div className="rc-diff">
                ...
                {row.left && !row.right && (
                  <span className="rc-diff-removed">−</span>
                )}
                {!row.left && row.right && (
                  <span className="rc-diff-added">+</span>
                )}
              </div>
      ...
      <div className="rc-summary-bar">
        <span>
          Left: <CategoryChips categories={leftRun.traceSummary.categories} />{" "}
          {Math.round(leftRun.traceSummary.duration / 1000)}s
        </span>
        <span>
          Right: <CategoryChips categories={rightRun.traceSummary.categories} />{" "}
  ...
  if (eventsLoading) {
    return (
      <div className="rc-container">
        <div className="rc-loading">
          <Loader2 size={14} className="rc-spinner" /> Loading trace events…
        </div>
      </div>
    );
  }
