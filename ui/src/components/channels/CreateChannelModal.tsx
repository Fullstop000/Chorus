...
interface Props {
  ...
  onCreated: (
    channel: Pick<import("../../data").ChannelInfo, "id" | "name">,
  ) => void;
...
export function CreateChannelModal({
  ...
  function addTeamMemberDraft() {
    const selected = availableMembers.find(
      (member) => member.name === pendingMemberName,
    );
    if (!selected) return;
    setTeamMembers((current) => [
      ...current,
      {
        ...
        role:
          pendingMemberRole.trim() ||
          (selected.member_type === "agent" ? "operator" : "observer"),
      },
    ]);
    setPendingMemberName("");
    setPendingMemberRole("operator");
  ...
  function removeTeamMemberDraft(memberName: string) {
    setTeamMembers((current) =>
      current.filter((member) => member.member_name !== memberName),
    );
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      onCreated({
        id: created.team.channel_id ?? undefined,
        ...
        >
          ...
          <FormField>
            ...
            <Input
              ...
              placeholder={
                mode === "channel" ? "e.g. engineering" : "e.g. eng-team"
              }
              value={name}
          ...
          {mode === "channel" ? (
            <FormField>
              <Label htmlFor="channel-modal-description">
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  >
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setPendingMemberRole(event.target.value)
                      }
                    />
                    <Button
                      type="button"
                      ...
                      {teamMembers.map((member) => (
                        ...
                        >
                          ...
                          <Button
                            ...
                            type="button"
                            onClick={() =>
                              removeTeamMemberDraft(member.member_name)
                            }
                          >
                            Remove
                          </Button>
