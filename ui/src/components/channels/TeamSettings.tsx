...
export function TeamSettings({
  ...
  const availableMembers = directory.filter(
    (entry) =>
      !members.some((member) => member.member_name === entry.member_name),
  );
  ...
  async function handleAddMember() {
    const selected = availableMembers.find(
      (entry) => entry.member_name === pendingMemberName,
    );
    if (!selected) return;
    setSaving(true);
    setError(null);
  ...
  async function handleDelete() {
    if (
      !window.confirm(
        `Delete team ${team.display_name}? This archives #${team.name}.`,
      )
    ) {
      return;
    }
            ...
            {members.map((member) => (
              <div key={member.member_name} className="team-settings-member">
                <div className="team-settings-member-copy">
                  <div className="team-settings-member-name">
                    {member.member_name}
                  </div>
