...
**Status:** Shipped via [#149](https://github.com/Fullstop000/Chorus/issues/149)
(PRs [#150](https://github.com/Fullstop000/Chorus/pull/150) /
[#152](https://github.com/Fullstop000/Chorus/pull/152) /
[#153](https://github.com/Fullstop000/Chorus/pull/153)).

What is in place:
- Persistent WebSocket between bridge and Platform — `/api/bridge/ws`
- Platform pushes desired runtime state via `bridge.target` frames; bridge
  reconciles by stopping/starting agents through its `AgentManager`
- Bridge pushes `agent.state` and `chat.ack` upstream
- `agents.machine_id` routes targets to the bridge that owns the agent;
  in `chorus serve`, an in-process bridge client registers under the
  installation's `local_machine_id` so single-machine deployments use
  the same code path as multi-machine ones
- `agents.paused` lets the platform express "stopped but desired" without
  removing the row from target; `agents.restart_seq` lets it ask the
  bridge to relaunch a runtime; `agents.pending_init_directive` carries
  one-shot decision-resume envelopes
- `AgentLifecycle` trait shrunk to observability — handlers no longer
  call `start_agent` / `stop_agent` / `notify_agent` / `resume_with_prompt`
