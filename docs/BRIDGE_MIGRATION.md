...
operation the discovery file is always present. If it's missing, restart
`chorus-server` to recreate it.
...
What is in place:
- `chorus-server` starts the bridge in-process; no separate standalone-bridge mode
...
If `bridge.json` does not exist, start `chorus-server` (the in-process bridge launches
automatically). If the file exists but the health check fails, the process recorded in
`bridge.json` may have died and left a stale file — see below.
...
Then restart `chorus-server` to start a fresh bridge.
...
chorus-server --port 3001 --bridge-port 4321
