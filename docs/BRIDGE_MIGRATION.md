...
`chorus-server` starts the shared bridge in the same process automatically:
...
```bash
chorus-server --port 3001 --bridge-port 4400
```

The standalone `chorus-server bridge-serve` subcommand is still available for users who want
to run the bridge in a separate process (for example, to share one bridge across multiple
chorus instances):
...
running. `chorus-server` starts the bridge in-process on startup, so under normal
operation the discovery file is always present. If it's missing, start the bridge
explicitly via `chorus-server bridge-serve` or restart `chorus-server`.
...
What is in place:
- `chorus-server` starts the bridge in-process; standalone `chorus-server bridge-serve` also
...
- `machine_id` (already in `config.toml`, generated by `chorus-server setup`) is used to
...
If `bridge.json` does not exist, start `chorus-server bridge-serve`. If the file exists but
...
Then restart `chorus-server` (which will start a fresh bridge) or start
`chorus-server bridge-serve` standalone.
