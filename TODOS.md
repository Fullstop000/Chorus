...
- [ ] **Opencode session-resume liveness guard.** Codex side landed via `codex_thread_file` in `src/agent/drivers/codex.rs` (`driver-resilience` branch, 2026-05-02): walks `~/.codex/sessions/<year>/<month>/<day>/rollout-*-<thread_id>.jsonl` newest-first, drops the resume on miss, mirrors claude's pattern. Opencode still needs the same: locate where opencode CLI persists its session state, add an analogous helper, gate the resume call on file existence. Discovered 2026-05-01 during decision-inbox dogfood.
