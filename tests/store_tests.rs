...
// --- Task proposal snapshot column migration (v2) ---
//
// These tests exercise the migration that adds the 5 snapshot columns + the
// `source_message_id` pointer to `task_proposals`. They bootstrap a bare
// v1-only schema (no v2 columns) and then run the migration explicitly so we
// can observe pre- and post-migration state. `make_store()` is unsuitable
// because `Store::open` applies every migration up front.
...
fn migration_add_snapshot_columns_is_idempotent() {
    ...
    chorus::store::migrations::migrate_add_task_proposal_snapshot_columns(&conn).unwrap();
    chorus::store::migrations::migrate_add_task_proposal_snapshot_columns(&conn).unwrap();
    let n: i64 = conn
        .query_row(
