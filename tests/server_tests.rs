...
struct MockLifecycle {
    ...
    /// Records every `resume_with_prompt(agent_name, envelope)` call.
    /// Decision-inbox tests assert against this to verify the
    /// resolution envelope reached the agent.
    resumed_with: Mutex<Vec<(String, String)>>,
    /// Channel id to return from `run_channel_id`. Decision-inbox tests
    /// set this so the create handler can infer the channel.
...
impl AgentLifecycle for MockLifecycle {
    ...
    fn run_channel_id(&self, agent_name: &str) -> Option<String> {
        self.run_channels.lock().unwrap().get(agent_name).cloned()
    }
}

impl MockLifecycle {
    /// Test helper: set the channel that `run_channel_id(agent_name)`
    /// should return. Decision-inbox tests call this before invoking
    /// `chorus_create_decision` so the create handler's channel
    /// inference returns a known id.
    fn set_run_channel_for_test(&self, agent_name: &str, channel_id: &str) {
        self.run_channels
            .lock()
            .unwrap()
...
// ───────────────────────────────────────────────────────────────────────
// Decision-inbox end-to-end
// ───────────────────────────────────────────────────────────────────────

/// E2e: agent creates a decision via the internal endpoint → human lists
/// it → human resolves it → server resumes the agent with the envelope
/// as the next-turn prompt.
///
/// This is the inner core of Day 5's meta-circular dogfood: it exercises
/// every layer of the v1 mechanism (validator + store + handlers + lifecycle)
/// against a single in-memory test fixture.
#[tokio::test]
async fn decision_round_trip_agent_creates_human_resolves_agent_resumed() {
    let (store, app, lifecycle) = setup_with_lifecycle();

    // Pre-condition: the agent has a runtime session and is on a
    // channel-triggered run. Both are required by the create handler's
    // channel-inference + session-routing contracts.
    store
        .record_session("bot1", "sess-xyz", "claude")
        .expect("record session");
    let general_channel = store
        .get_channel_by_name("general")
        .unwrap()
        .expect("general channel exists")
        .id;
    lifecycle.set_run_channel_for_test("bot1", &general_channel);

    // Step 1: agent calls chorus_create_decision (via the bridge ->
    // internal handler).
    let payload = serde_json::json!({
        "headline": "PR #121 ready: archived-channel del/join fix",
        "question": "How do you want to land this?",
        "options": [
            {"key": "a", "label": "Merge as-is", "body": "Squash and merge to main."},
            {"key": "b", "label": "Request changes", "body": "Add the e2e test first."},
        ],
        "recommended_key": "a",
        "context": "## Why now\nCI green at 20:40Z."
    });
    ...
    let create_body: serde_json::Value =
        serde_json::from_slice(&body_to_bytes(create_resp).await).unwrap();
    let decision_id = create_body
        .get("decision_id")
        .and_then(|v| v.as_str())
        .expect("decision_id in response")
        .to_string();
    assert!(!decision_id.is_empty());

    // Step 2: human lists open decisions.
    let list_resp = app
        .clone()
        .oneshot(
            Request::builder()
    ...
    assert_eq!(decisions[0]["agent_id"], "bot1");
    assert_eq!(decisions[0]["session_id"], "sess-xyz");

    // Step 3: human resolves with picked_key="a" + a note.
    ...
    // Step 4: lifecycle.resume_with_prompt was called with bot1 + the
    ...
    assert!(envelope.contains(&decision_id));
    assert!(envelope.contains("PR #121 ready"));
    assert!(envelope.contains("How do you want to land this?"));
    assert!(envelope.contains("picked: [a]") || envelope.contains("[a]"));
    assert!(envelope.contains("Merge as-is"));
    assert!(envelope.contains("Squash and merge to main."));
    assert!(envelope.contains("note: lgtm"));
    assert!(envelope.contains("Please proceed."));

    // Step 5: subsequent open-decisions list must be empty (status=open).
    let list_resp_after = app
        .clone()
        .oneshot(
            Request::builder()
...
async fn decision_resolve_double_pick_returns_409() {
    ...
    let body: serde_json::Value = serde_json::from_slice(&body_to_bytes(create).await).unwrap();
    let id = body["decision_id"].as_str().unwrap().to_string();
    ...
    let r1 = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!("/api/decisions/{id}/resolve"))
                .header("content-type", "application/json")
                .body(Body::from(
...
async fn decision_create_without_active_channel_returns_400() {
    ...
    store
        .record_session("bot1", "sess-xyz", "claude")
        .expect("record session");
    // Note: deliberately NOT calling set_run_channel_for_test.

    let payload = serde_json::json!({
        "headline": "h", "question": "q",
        "options": [
            {"key": "a", "label": "A", "body": "x"},
            {"key": "b", "label": "B", "body": "y"},
        ],
        "recommended_key": "a",
        "context": ""
    });
...
async fn decision_resolve_unknown_picked_key_returns_400() {
    ...
    // Pick "z" which isn't in the options.
    let resp = app
        .oneshot(
            Request::builder()
                .method("POST")
...
async fn body_to_bytes(resp: axum::http::Response<axum::body::Body>) -> Vec<u8> {
    axum::body::to_bytes(resp.into_body(), 1_000_000)
        .await
        .unwrap()
        .to_vec()
